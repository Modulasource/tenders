...
<%
	...
	Vector<PublicationJoue> vPublicationsJoue = PublicationJoue.getAllPublicationJoue();
%>
...
<% 
	...
	{
%>
<a href="<%= response.encodeURL("modifierPublicationFormJoue.jsp?sAction=create"
...
<table class="liste" summary="Les publications">
	...
	<%
	for (int i=0; i < vPublicationsJoue.size();i++)
	{
		PublicationJoue publi = vPublicationsJoue.get(i);
		int j = i % 2;
		...
		String sUrlAfficherPublication 
			= response.encodeURL(
					rootPath 
					+ "desk/export/publication/afficherPublicationJoue.jsp?"
					+ "iIdPublicationJoue=" + publi.getIdPublicationJoue()
	...
	<tr class="liste<%=j%>"
		...
		<td style="text-align:right"><a href="<%= response.encodeURL( 
			...
			rootPath + "desk/export/modifierPublicationJoue.jsp?sAction=remove&amp;iIdExport="
