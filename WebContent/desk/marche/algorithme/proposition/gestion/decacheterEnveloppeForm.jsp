...
<%  
    ...
    //pour les tests marche = new Marche(768);
    htmlDecachetage.init(request,marche);
    int iTypeEnveloppe=htmlDecachetage.iTypeEnveloppe;
    htmlDecachetage.iTypeApplet = AppletConstant.APPLET_DECACHETAGE_ENVELOPPE;
    String sTitle = htmlDecachetage.sTitle;
    session.setAttribute("iTypeEnveloppe",htmlDecachetage.iTypeEnveloppe);  
...
function initUI(){
    ...
    }else if(!htmlDecachetage.bAddCandPapier && !htmlDecachetage.bEnveloppesDecachetees){
        //show decachetage que si on a pas encore décacheté
    ...
    $("decipher").onclick = function(){
        if(!bIsAllCandidaturePapier){
	        var cert = isCertificateSelected();
	        if(isNotNull(cert)){
	            cert = cert.parseJSON();
	            if(confirm("Vous etes sur le point de décacheter avec le certficat "+cert.sAlias+".\nVeuillez confirmer cette opération.")){
	                decipher();
	            }
	        }else{
    ...
    /** pas de candidatures */
    if(jsonCand.length==0){
    	$("decipher").innerHTML = "Valider cette étape sans décacheter";
    }
...
function endProcess(){
    if(countDecipher == jsonCand.length){
         ...
         location.href = "<%= response.encodeURL(rootPath + sUrlTraitement+"?iIdAffaire="+iIdAffaire+"&iTypeEnveloppe="+iTypeEnveloppe+"&iIdNextPhaseEtapes="+iIdNextPhaseEtapes)%>";
          }
}
...
<div id="divAnonymat" style="display:none;text-align:center;">
    ...
    <div style="padding:5px;">
        <form name="formulaire" id="formulaire" method="post" action="<%= 
        	response.encodeURL("decacheterEnveloppe.jsp?" )%>" >
...
<button type="button" id="verify" onclick="parent.addParentTabForced('Chargement...','<%= 
    ...
    alt="Tester" style="vertical-align:middle;margin-right: 5px;" />Tester votre poste</button>
<button type="button" name="poursuivre" id="poursuivre"
        ...
        onclick="Redirect('<%= response.encodeURL(rootPath 
                ...
                %>')" ><img src="<%= rootPath %>images/icons/arrow_right.gif" 
                        alt="poursuivre" 
                        style="vertical-align:middle;margin-right: 5px;" />Poursuivre la procédure</button>
