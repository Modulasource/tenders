<%
    ...
    for (int i = 0; i < vLots.size() ;i ++)
    {
        ...
        String sReference = "";
        if(vLots.size() == 1) sReference = "marché ref." + marche.getReference();
...
<%
    ...
    for (int j = 0; j < vEnveloppeALotAll.size() ;j ++)
    {
        ...
        for (int k = 0; k < vEnveloppeALot.size() ;k ++)
        {
            ...
            {
                /**
                 * L'enveloppe peut etre affichée car elle appartient à l'ensemble
        ...
        String sDateCachetageCandidature = "Candidature non cachetée";
        
        String sStatut = "Indéfini";
        if(oEnveloppeALot != null)
        {
            try{sStatut = oEnveloppeALot.getStatutLibelleValue(EnveloppeALot.ID_STATUS_RECEVABLE);}
            catch(Exception e){sStatut = "Indéfini";}
...
%>
                <tr class="liste<%=j%2%>" onmouseover="className='liste_over'" 
                    ...
                    <td style="width:10%"><strong><%= sStatut.equalsIgnoreCase("")?"Indéfini":sStatut %></strong></td>
                    <td style="width:10%;text-align:right">
                    <%=(bHorsDelai)?"<img src='"+sIconeHorsDelai+"' alt='Hors délai' title='Hors délai'/> ":"" %>
...
<%
             ...
             * car elles seront considérées comme anti-datées.
             */
            ...
            for(int iEnvAPJ=0;iEnvAPJ<vEnveloppeAPJ.size();iEnvAPJ++)
            {
                ...
                {
                    /**
                     * La pièces n'est plus chiffrée, elle peut etre affichée à l'AP. Et aussi téléchargeable.
                     */
                    ...
                    <tr class="liste<%=j%2%>" onmouseover="className='liste_over'" 
                        ...
                        <td style="width:15%">Pièce jointe</td>
