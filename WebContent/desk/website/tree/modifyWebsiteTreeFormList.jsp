...
Vector<WebsiteTree> vWebsiteTree = WebsiteTree.getAllEditable();
if (vWebsiteTree.size()<1){
	out.println("<html><head></head><body>Vous n'avez accès à aucune page.</body></html>");
	return;
}

long lId = HttpUtil.parseLong("lIdWebsiteTree", request, vWebsiteTree.firstElement().getId());
...
tinyMCE.init({
		...
		theme_advanced_buttons2 : "strikethrough,justifyleft,justifycenter,justifyright,justifyfull,bullist,numlist,|,outdent,indent,blockquote,|,formatselect,undo,redo,|,sub,sup,|",
...
<form name="formList" id="formList" method="get" action="<%= response.encodeURL("modifyWebsiteTreeFormList.jsp") %>">
	<div style="margin: 2px;margin-top:5px;text-align: center;">
		Liste des pages modifiables :
		<select id="page_list" name="lIdWebsiteTree">
			<option value="0">- Sélectionner la page à modifier dans cette liste -</option>
			<% for(int i=0;i<vWebsiteTree.size();i++){
				out.println("<option value=\""+vWebsiteTree.get(i).getId()+"\">"
						+vWebsiteTree.get(i).getTitle()
						+" (n°"+vWebsiteTree.get(i).getId()+")</option>");
			  }
			%>
		</select>
	</div>
