...
<%
    ...
    String sTitle = "Display all Folder";
...
engine.onBeforeSearch = function(){
	...
	if (isNotNull(sKeyword)){
		...
		var words = sKeyword.split(new RegExp("\\W", "g"));
		/*
		var realWords = [];
		for (var z=0; z<words.length; z++){
			if (words[z].trim().length>3) realWords.push(words[z].trim());
		}
		sKeyword = "*"+realWords.join("* *")+"*";
		var wordsCount = realWords.length;
		*/
		var wordsCount = words.length;
		
		//engine.addWhereClause("MATCH (f.name, f.description, f.reference) AGAINST (? IN BOOLEAN MODE)=?", [sKeyword, wordsCount]);
		engine.addWhereClause("MATCH (f.name, f.description, f.reference) AGAINST (? IN BOOLEAN MODE)", [sKeyword]);
	}
	if (ged_categories.getValue()>0){
		var sReq = "((gcs.id_type_object=<%= ObjectType.GED_FOLDER %> AND gcs.id_reference_object=f.id_ged_folder) OR ";
		sReq += "(gcs.id_type_object=<%= ObjectType.GED_DOCUMENT %> AND gcs.id_reference_object=f.id_ged_document))";
		sReq = "gcs.id_type_object=<%= ObjectType.GED_FOLDER %> AND gcs.id_reference_object=f.id_ged_folder"; 
		engine.addTable("ged_category_selection gcs", sReq, []);
		engine.addWhereClause("gcs.id_ged_category=?", [ged_categories.getValue()]);
	}
	return true;
};
...
function populate(){
	...
	ged_categories = new gedCategoryComponentExplorer('gedCategoryCpt', <%=GedCategory.getJSONArrayFromParent(0)%>, engine, <%=sessionLanguage.getId ()%>);
...
<div style="padding:15px">
    ...
    <div class="left">
        ...
        response.encodeURL("modifyFolderForm.jsp?sAction=create") 
                %>';" ><img src="<%= rootPath
                %>images/icons/add.gif" alt="Add" style="margin-right: 5px;" /><%=locButton.getValue (5, "Créer un dossier")%></button>

        <button type="button" onclick="doUrl('<%=
        response.encodeURL(
        		rootPath + "desk/ged/import/email/displayMailList.jsp") 
                %>');" ><img src="<%= rootPath
                %>images/icons/add.gif" 
                alt="Add" 
                style="margin-right: 5px;" /><%=locButton.getValue (32, "Créer un dossier à partir d'un email")%></button>
