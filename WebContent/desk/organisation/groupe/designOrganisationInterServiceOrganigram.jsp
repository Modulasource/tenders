...
<%
	...
	/**
	 * Ici les noeuds sont des services
	 */
	Vector vService = OrganisationService.getAllFromIdOrganisation( (int) item.getIdReferenceObject());
		...
		OrganisationService service = (OrganisationService)
			OrganisationService
				.getCoinDatabaseAbstractBeanFromId(
					node.getIdReferenceObject(), vService);
...
%>
   ...
   // ]]>
	...
	<div id="fiche_footer">
		<input type="button" value="Retour" onclick="javascript:Redirect('<%=
		...
		<input type="button" value="Enregistrer Position" onclick="javascript:storePosition()" />
	</div>
