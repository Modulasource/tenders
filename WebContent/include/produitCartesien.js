function vider(formListe){
	formListe.options.length = 1;
}

function viderTotal(formListe){
	formListe.options.length = 0;
}
...
function selectionnerOption2NiveauxId(liste, listeValeurSelectionneeID, listeSuivante, listeValeurSuivanteSelectionneeID)
{
	var l1 = eval("document.formulaire." + liste);
	var l2 = eval("document.formulaire." + listeSuivante);

	
	selectOptionById(liste, listeValeurSelectionneeID);
	
	remplir(listeSuivante, niveau2(l1.options[l1.options.selectedIndex].value));
	
	selectOptionById(listeSuivante, listeValeurSuivanteSelectionneeID);
}
